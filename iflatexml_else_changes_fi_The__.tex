...
\section{Classification for SSVEP}
\label{sec:mdm}
\iflatexml\else \changes{ \fi 
...
For estimation of centers of classes in step~\ref{op:class_center}, outliers matrices are removed to have a reliable mean estimation using Algorithm~\ref{alg:center_estimation} \cite{barachant2013riemannian}. TODO: It is used with a z-score threshold $z_{th} = 2.5$ and a number of iterations $L=2$.
...
\label{alg:center_estimation}
	...
	Inputs: $L$, the number of iterations and $z_{th}$, the z-score threshold. \\
	...
	\State \quad Compute mean $m_l$ and standard deviation $\sigma_l$ of distances $\{d_j\}$
	\State \quad \textbf{for} $j$ \in \mathcal{J} \textbf{do}
	\State \quad \quad Compute z-score $z_j = (d_j - m_l) / \sigma_l$
	\State \quad \quad \textbf{if} $z_j>z_{th}$ \textbf{then} $\mathcal{J} = \mathcal{J} \backslash j$
	\State \textbf{end}
