...
Often the symmetrization consists in an averaging of left- and right-sided divergence [todo: reference].
\iflatexml\else } \fi
...
If the two probability distributions are pictured as two different ways of piling up a mass of sand, then the Wasserstein distance can be seen as the optimal cost of involved in transporting sand in from one piling to another [ref: Villani 2008].

Let P(\mathcal{X}) and P(\mathcal{Y}) two spaces of probability measures,
the optimal transport between two masses (or probability distributions) $\eta \in P(\mathcal{X})$ and $\nu \in P(\mathcal{Y})$ is defined as [ref: Villani 2008]:
\begin{equation}
...
\label{eq:wasser}
\end{equation}
Using the Fr\'{e}chet distance, and following the development in [ref: Barbaresco 2011], the Wasserstein distance of multivariate Gaussian measure $\Eta$ and $\Nu$ with means $\mu_X$ and $\mu_Y$, and covariance matrices $\Sigma_X$ and $\Sigma_Y$, is expressed as:
\begin{equation}
d_W(N(\mu_X, \Sigma_X),N(\mu_Y, \Sigma_Y)) = |\mu_X -\mu_Y|^2 + \tr\left[ \Sigma_X \right] + \tr\left[ \Sigma_Y \right] - 2 \tr\left[ \left( \Sigma_X^{1/2} \Sigma_Y \Sigma_X^{1/2} \right)^{1/2} \right] 
\label{eq:dist_wass}
\end{equation}
...
\begin{table}[h]
  ...
  \begin{tabular}{ l | c | c | c |}
    ...
    \multicolumn{1}{|l|}{Wasserstein} & $\distW = \tr \left( \P_1 + \P_2 - 2 \left( \P_1^{1/2} \P_2 \P_1^{1/2} \right)^{1/2} \right)$ & Eq. 6.2 in [ref: Agueh 2011]  & \cite{barbaresco_geometric_2011} [ref: Agueh 2011] \rule[-5pt]{0pt}{18pt} \\ \hline
  \end{tabular}
